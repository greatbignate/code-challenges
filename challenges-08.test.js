...
const filterStringsWithVowels = (input) => {
  // Solution code here...
  return input.filter(word => word.search(/[AaEeIiOoUu]/) !== -1);
};
...
const notInFirstArray = (forbiddenValues, input) => {
  // Solution code here...
  return input.filter(i => i!== forbiddenValues.filter(n => n === i)[0]);
};
...
const getBaseStatGreaterThan = (input, minBaseStat) => {
  // Solution code here...
  return input.filter(ability => ability.baseStat > minBaseStat);
};
...
const getStatName = (input, minBaseStat) => {
  // Solution code here...
  return (input.filter(ability => ability.baseStat > minBaseStat)).map(instance => instance.stat.name);
};
...
const getCharactersWithoutChildren = (input) => {
  // Solution code here...
  return input.filter(family => family.children === undefined);
};
...
const evenOddNumericValues = (input) => {
  // Solution code here...
  return input.filter(input => typeof input === 'number').map(number => number%2 ? 'odd' : 'even');
};
