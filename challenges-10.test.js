...
const count = (target, input) => {
  ...
  return input.reduce((subArr, curArr)=> subArr.concat(curArr)).filter(i => i === target).length;
  }
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((subArr, curArr)=>subArr.concat(curArr)).reduce((acc, val) => acc+val);
};
...
let findShortest = (data) => {
  // Solution code here...
  data.reduce((acc, val) => {
    console.log(acc, val.height, parseInt(val.height));
    if (parseInt(acc.height) > parseInt(val.height)) {
      acc = val;
      return acc.name;
    } else {
      return acc.name;
